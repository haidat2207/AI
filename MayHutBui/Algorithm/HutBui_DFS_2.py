...
class Node:
    def __init__(self, state, x, y, node_parent=None, action=None, step=0):
        self.state = state
        self.x = x
        self.y = y
...
def DFS(initial_state, goal_state, x, y):
    if initial_state[x][y] == 1:
        initial_state[x][y] = 0
    root = Node(initial_state, x, y, node_parent=None, action=None, step=0)
...
def print_result(node):
    ...
    while node:
        ...
        path.append((node.action, node.state,node.x, node.y, node.step))
        node = node.parent

    path.reverse()

    for action, state, x, y, step in path:
        print(f"Bước: {step}")
...
# result = DFS(initial_state, goal_state, x, y)
