...
def build_unobservable_belief(matrix, robot_x, robot_y, estimate_count):
    belief = []
    ...
    # Ma trận đầu tiên lấy từ ma trận người dùng nhập
    belief.append(
    ...
    # Các ma trận còn lại là ma trận ước lượng random
    for i in range(estimate_count - 1):
...
def UnobservableSearch(initial_belief, goal_state, n, m):
...
# initial_state = [make_initial_possibility(initial_state_1, x, y), make_initial_possibility(initial_state_2, x, y)]

# result = UnobservableSearch(initial_state, goal_state, n, m)
